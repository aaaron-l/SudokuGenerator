...
The first thing I did was make a new class called Game.
In that class, I used an ArrayList to keep track of masked cells, a board and a generator attribute which contains the SudokuGenerator object and its corresponding board. Also in the game class is the mask function, which accepts an int as a parameter to decide how many cells are masked. I also made a Masked class which was used to record the row, column, and value of a cell which was replaced by a 0 (masked). Using these attributes, I made an input class which checks if the row and column the user entered is empty, and checks the value the user entered and compares it with the masked cell's value. If the user entered the correct value, that value is removed from the ArrayList of the masked values, which acts as a win condition where the user wins once there is no more values in the ArrayList as demonstrated by the check function.

In the Masked class, I overrode the .equals() function so calling ArrayList.contains() will check the objects for my code correctly. In the main class, I changed the code to previously only printing a solved sudoku board into a user input reciever through scanner, allowing the user to actually play a sudoku game and choose a difficulty.